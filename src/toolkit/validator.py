...
def validation_calc(tokens):
    if operation_before_first_operand(tokens)[0]:
        print(
            f"Operation before the first operand: {operation_before_first_operand(tokens)[1]}", file=sys.stderr)
        sys.exit(2)
    if sequential_operations(tokens)[0]:
        print(
            f"Sequential Operations: {sequential_operations(tokens)[1]}", file=sys.stderr)
        sys.exit(2)
    ...
    if no_operand_after_operation(tokens)[0]:
        print(
            f"No operand after operation: {no_operand_after_operation(tokens)[1]}", file=sys.stderr)
        sys.exit(2)
...
def validation_convert(Value, From, To):
    if unknown_metrics(Value, From, To)[0]:
        print(
            f"Unknown metrics: {unknown_metrics(Value, From, To)[1]}", file=sys.stderr)
        sys.exit(2)
    elif wrong_metrics_type(Value, From, To)[0]:
        print(
            f"Cant convert between different Types: from {From} to {To}", file=sys.stderr)
        sys.exit(2)
